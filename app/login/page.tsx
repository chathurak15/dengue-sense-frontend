...
import { useAppStore, toFrontendUser } from "@/stores/app-store";
...
function LoginForm() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const userDTO = await apiLogin({ email, password });
      const user = toFrontendUser(userDTO);
      setUser(user);
    ...
    <div className="relative flex min-h-screen items-center justify-center bg-background px-4">
      ...
      <Card className="w-full max-w-md border-border">
        <CardHeader className="space-y-3 text-center">
          ...
          <CardDescription>Sign in to the DengueSense portal</CardDescription>
        </CardHeader>
